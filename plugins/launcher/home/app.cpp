...
{
    qDebug() << "[Home] starting program: " << app;
    ...
    if(window->isModal()) qDebug() << "app is modal: " << app;
    if(window->isTopLevel()) qDebug() << "app is Top Level: " << app;
    if(!window->isExposed()) qDebug() << "window not exposed: " << app;
    if(!window->isActive()) qDebug() << "window not active: " << app;
    if(window == nullptr) {
        qDebug() << "error opening application: " << app;
        return;
    }
...

{
    ...
    if (!inputFile.isOpen()){
        ...
        qDebug() << "[Home] invalid Desktop Entry " << inputFile.errorString();
        return;
    }
      ...
      line = stream.readLine())
    {
        ...
        if(line.contains("Exec")){
            ...
            args.removeAt(0);
            this->args_ = args;
            //TODO: remove flags and add to separate field
            qDebug() << this->exec_;
        }
    ...
    for(QString dir_name : xdgArr)
    {
        ...
        while ( themes.hasNext() ) {
            ...
            while ( resolutions.hasNext() ) {
                ...
                qDebug() << "Getting resolution" << resolution;
                ...
                while ( subdirs.hasNext() ) {
                    ...
                    qDebug() << "Searching for " << xdg_png_path;
