...
WindowManager* WindowManager::Create(const string& display_str) {
  ...
  if (display == nullptr) {
     qCritical() << "Failed to open X display " << XDisplayName(display_c_str);
    return nullptr;
  }
...
void WindowManager::Run() {
  ...
  //   exit gracefully if another window manager is already running.
  {
    ...
    if (wm_detected_) {
      ...
      this->isRunning = true;
      qDebug() << "No window manager running, proceeding...";
    }
  }
  //   b. Set error handler.
  ...
  if(returned_root != root_) qDebug() << "[ERROR] root mismatch";
  ...
  for (;;) {
    ...
    QMessageBox msgBox;
    QString qstr = QString::fromStdString(ToString(e));
    msgBox.setText("Received event: " + qstr);
    msgBox.exec();
