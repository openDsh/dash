            ...
            if (canbus == "RX1")
            {
                ...
                if (id == "0208")
                {
                    if((canMsg[0] == 0x01) && (canMsg[1] == 0x17) && (canMsg[2] == 0x00) && bloccato == false){
                        usleep(200000);
                        bus->socket.write(QString("CAN USER TX CH1 0208 081601000000\r\n").toUtf8()); //turn right
                        bus->socket.flush();
                        usleep(200000);
                        bus->socket.write(QString("CAN USER TX CH1 0208 011700000000\r\n").toUtf8()); //button press
                        bus->socket.flush();
                        usleep(70000);
                        bus->socket.write(QString("CAN USER TX CH1 0208 001700000000\r\n").toUtf8()); //button release
                        bus->socket.flush();
                        usleep(200000);
                        bus->socket.write(QString("CAN USER TX CH1 0208 0816FF000000\r\n").toUtf8()); //turn left
                        bus->socket.flush();
                        usleep(30000);
                        bus->socket.write(QString("CAN USER TX CH1 0208 0816FF000000\r\n").toUtf8()); //turn left
                        bus->socket.flush();
